...
/** Agent skills catalogue (devboard:skills-import) + per-task choice in the modal + devboard:check output. */
class SkillsTest extends TestCase
{
    ...
    {
        parent::setUp();
        config(['devboard.skills_file' => sys_get_temp_dir().'/devboard-skills-test-'.getmypid().'.json']);
        @unlink(Skills::path());
    ...
    public function test_import_from_file_and_stdin_like_json(): void
    {
        ...
        $this->artisan('devboard:skills-import', ['--file' => $file])->expectsOutputToContain('2 skills imported')->assertSuccessful();
        $all = Skills::all();
        $this->assertSame(['frontend-design', 'tdd'], array_keys($all));
        $this->assertSame('Test-driven development', $all['tdd']['description']);

        $this->artisan('devboard:skills-import', ['--file' => '/nonexistent.json'])->assertFailed();
    }
    ...
    public function test_modal_toggles_skills_and_check_prints_them(): void
    {
        ...
        $this->artisan('devboard:check')->expectsOutputToContain('🧩 skills to activate for this task (Skill tool): tdd, frontend-design')->assertSuccessful();
