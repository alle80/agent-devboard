...
/** Per-todo statistics: agent working time (🔧 intervals) and tokens reported via griglia:check. */
class TodoStatsTest extends TestCase
{
    ...
    public function test_working_time_accumulates_over_intervals_and_tokens_add_up(): void
    {
        ...
        $this->artisan('griglia:check')->expectsOutputToContain('⏱ working since 2026-08-19T10:00:00+00:00 (5m 00s this interval)')->assertSuccessful();

        // A question closes the interval (the user's thinking time is not agent time)
        $this->artisan('griglia:check', ['--ask' => $this->todo->id, '--q' => ['Which?'], '--tokens-in' => 1200, '--tokens-out' => 300])->assertSuccessful();
        $this->todo->refresh();
        ...
        $this->artisan('griglia:check', ['--done' => $this->todo->id, '--comment' => 'ok', '--tokens-in' => 800, '--tokens-out' => 200])
        ...
        $this->artisan('griglia:check', ['--all' => true])->expectsOutputToContain('📊 ⏱ 7m 30s')->assertSuccessful();
    }
