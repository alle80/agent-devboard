...
| `access_gate` | `GRIGLIA_ACCESS_GATE` | — | Modalità server: chi può aprire la board. Se il modello utente ha `canAccessDevboard(): bool` decide quello; altrimenti questa ability del Gate (per esempio 'access-devboard') se impostata; altrimenti chiunque sia autenticato. |
...
| `default_list_name` | — | `'My list'` | Nome della lista di default creata per un utente nuovo |
