...
| `admin_gate` | `GRIGLIA_ADMIN_GATE` | — | Server mode: Gate ability deciding who may ADMINISTER the board (settings, agent context, theme packs). It is consulted after `canManageGriglia(): bool` on the user model (the pre-rename `canManageDevboard()` is still honoured) and before `admins`. |
