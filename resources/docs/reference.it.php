...
return [
    ...
    'text' => [
        ...
        'Server mode: Gate ability deciding who may ADMINISTER the board (settings, agent context, theme packs). It is consulted after `canManageGriglia(): bool` on the user model (the pre-rename `canManageDevboard()` is still honoured) and before `admins`.' => 'Modalità server: ability del Gate che decide chi può AMMINISTRARE la board (impostazioni, contesto dell\'agente, pacchetti di temi). Si consulta dopo `canManageGriglia(): bool` sul modello utente (il vecchio `canManageDevboard()`, da prima della rinomina, è ancora onorato) e prima di `admins`.',
