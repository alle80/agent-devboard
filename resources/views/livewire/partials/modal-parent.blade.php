    ...
    <details class="{{ $boxClass }} modal-parent group" open>
        ...
        <div class="mt-2 space-y-2 text-[0.95em]">
            ...
            @if ($todo->parent->claude_comment)
                <p class="{{ $textClass }} whitespace-pre-wrap break-words opacity-80"><span class="font-bold">{{ __('devboard::t.agent_box', ['agent' => \Alle80\Devboard\Agent::name()]) }}:</span> {{ $todo->parent->claude_comment }}</p>
            @endif
