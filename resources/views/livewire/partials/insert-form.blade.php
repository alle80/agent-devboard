<form wire:submit="saveInsert" x-data="{ len: 0 }" class="tl-card my-2 flex items-center gap-2 px-3 py-2">
    ...
    >
    <x-griglia::mic class="tl-check tl-display shrink-0 px-2 py-1.5" within="form" target="input" />
    ...
    <button type="submit" class="tl-check tl-display shrink-0 cursor-pointer px-3 py-1.5 transition active:translate-y-px">{{ __('griglia::t.ok') }}</button>
    <button type="button" wire:click="cancelInsert" class="tl-check tl-display shrink-0 cursor-pointer px-3 py-1.5 transition active:translate-y-px"><x-griglia::icon name="close" /></button>
</form>
